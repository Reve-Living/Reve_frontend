...
const CategoryPage = () => {
  ...
  const initialSnapshot = useMemo(() => {
    ...
    const handoffMatches =
      ...
      !linkedBedSize &&
      !debouncedServerFilterParamsKey;
    if (!handoffMatches || !handoff || !Array.isArray(handoff.products)) return null;

    return {
      ts: Date.now(),
      category: handoff.category,
      subcategories: handoff.subcategories,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        // Keep filtering in one response so the product grid, result count,
        // and pagination always update from the same filter selection.
